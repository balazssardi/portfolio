    ...
    <>
      <div className="h-9 navbar w-screen flex flex-row justify-between items-center overflow-hidden z-10 text-white">
        <div className="flex flex-row relative items-center z-20 gap-1">
          <button
            className={
              "px-12 h-20 w-36 italic font-bold text-xl shadow-2xl rounded-r-full " +
              (menu ? "startbuttona" : "startbutton")
            }
          ...
          {context.map(
            (item, index) =>
              (item.active || item.onTray) && (
                <div
                  key={index}
                  onClick={() => toggleTray(index, setContext, context)}
                  className={
                    "text-white text-xs items-center flex flex-row gap-1 cursor-pointer " +
                    (!item.isFront ? "trayelement" : "trayelementactive")
                  }
                >
        ...
        <div className="trayright flex items-center justify-self-end gap-1">
          <Image src={volume} alt="Volume icon" width={16} />
          <Image src={trayicon} alt="Trayicon" width={16} height={16} />
          <Time />
        </div>
      </div>
      <div
        className={
          "absolute w-96 bg-white transition-all text-white z-10 h-128 rounded-t-lg overflow-hidden flex flex-col shadow-[3px_1px_5px_rgba(0,0,0,0.8)] bottom-9 " +
          (menu ? "block" : "hidden")
        }
      >
        <div className="h-20 start flex items-center px-2 gap-2">
          ...
          />
          <p className="text-xl">Yoour Profile</p>
        </div>
        <div className="flex-1 flex flex-row">
          <div className="w-1/2 flex flex-col p-1 gap-0.5">
            {context.map((item, index) => (
              <div
                key={index}
                className="p-1 flex flex-row gap-2 items-center text-blue-800 hover:bg-[rgb(49,106,197)] hover:text-white cursor-pointer"
                onClick={() => openWi(index)}
              >
          ...
          <div className="w-1/2 bg-blue-200 border-l-2 border-blue-200 flex flex-col p-1 gap-0.5">
            ...
            <div className="p-1 flex flex-row gap-2 items-center text-blue-800 hover:bg-[rgb(49,106,197)] hover:text-white cursor-pointer">
              <Image src={x} alt="X icon" width={28} height={28} />
              <p className="text-sm font-bold">X</p>
            </div>
            <hr className="w-5/6 self-center my-2 border-blue-300" />
            <div className="p-1 flex flex-row gap-2 items-center text-blue-800 hover:bg-[rgb(49,106,197)] hover:text-white cursor-pointer">
              <Image src={aboutme} alt="About me icon" width={28} height={28} />
        ...
        <div className="h-12 startbot flex items-center justify-end px-2">
          <button className="flex flex-row gap-2 items-center">
            <Image src={shutdown} alt="X icon" width={28} height={28} />
            Kapcsolat
          </button>
        </div>
      </div>
